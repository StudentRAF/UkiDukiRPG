...
) : DebuffEffect(nameof(MagicDecreaseEffect), duration, scheduler)
{
    ...
    public override void Apply(IHero attacker, IHero defender)
    {
        ...
        m_Decrease = (int)((defender.EffectiveStatBlock.Magic.Level - m_BaseDecrease) * (1 - m_DecreaseFactor) * attackerModifier * defenderModifier);

        defender.ProgressionStatBlock.Magic.Descend(m_Decrease);
    ...
    {
        defender.ProgressionStatBlock.Magic.Ascend(m_Decrease);
    }
}
